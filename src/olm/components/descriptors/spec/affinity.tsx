    ...
    <div className="co-affinity-term">
      {showRemoveButton && (
        <Button
          ...
          variant="link"
        >
          {t('console-shared~Remove {{item}}', { item: type })}
        </Button>
      )}
...
export const NodeAffinity: FC<NodeAffinityProps> = ({ affinity, onChange, uid = '' }) => {
  ...
  const updateRequiredRules = ({ selector }: NodeAffinityRuleType, atIndex: number) =>
    onChange?.({
      ...affinity,
      requiredDuringSchedulingIgnoredDuringExecution: {
  ...
  const updatePreferredRules = (
    { selector: preference, weight }: NodeAffinityRuleType,
    atIndex: number,
  ) =>
    onChange?.({
      ...affinity,
  ...
  return (
    <DescriptionList>
      <DescriptionListGroup>
        <Tooltip
          content={t(
            'console-shared~Required rules must be met before a pod can be scheduled on a node.',
          )}
        >
          <DescriptionListTerm>
            {t('console-shared~Required during scheduling, ignored during execution')}
          </DescriptionListTerm>
        </Tooltip>
        <DescriptionListDescription>
      ...
      <DescriptionListGroup>
        <Tooltip
          content={t(
            'console-shared~Preferred rules specify that, if the rule is met, the scheduler tries to enforce the rules, but does not guarantee enforcement.',
          )}
        >
          <DescriptionListTerm>
            {t('console-shared~Preferred during scheduling, ignored during execution')}
          </DescriptionListTerm>
        </Tooltip>
        <DescriptionListDescription>
          ...
          <div>
            <Button
              ...
              variant="link"
            >
              {t('console-shared~Add preferred')}
            </Button>
          </div>
        </DescriptionListDescription>
      </DescriptionListGroup>
...
export const PodAffinity: FC<PodAffinityProps> = ({ affinity, onChange, uid = '' }) => {
  ...
  const updateRequiredRules = (
    { podAffinityTerm: next }: PodAffinityRuleType,
    atIndex: number,
  ) =>
    onChange?.({
      ...affinity,
  ...
  const updatePreferredRules = (next: PodAffinityRuleType, atIndex: number) =>
    onChange?.({
      ...affinity,
      preferredDuringSchedulingIgnoredDuringExecution: preferredRules.map((current, index) =>
        index === atIndex ? next : current,
...
export type NodeAffinityRuleProps = {
  key: string;
  onChange?: (rule: NodeAffinityRuleType) => void;
  onClickRemove?: () => void;
  rule: NodeAffinityRuleType;
...
export type PodAffinityRuleProps = {
  key: string;
  rule: PodAffinityRuleType;
  onChange?: (rule: PodAffinityRuleType) => void;
  onClickRemove?: () => void;
  showRemoveButton?: boolean;
  type: AffinityRuleType;
};
